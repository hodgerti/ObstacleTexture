...
void vertex()
{
	...
	if (RESTART) {
		...
		TRANSFORM *= rotationMatrix(vec3(0.0, 0.0, 1.0), initial_angle);
		// start moving
		vec2 firing_direction = ang2vec(initial_angle);
		VELOCITY.xy = firing_direction * speed;
	} else {
		// apply gravity
		VELOCITY.y += gravity * DELTA;
			
		// check if inside obstacle next frame
		// get pos
		ivec2 world_pos;
		...
		if (non_obstacle) {
			// stop moving if inside square next frame (this looks better than stopping when we are in the obstacle since textures usually overlap obstacles)
